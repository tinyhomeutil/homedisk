...
IFore action = (IFore)ActionFacotry.getInstance().getAction(SwitchAction.FORE_IMAGE_LIST);
Pages<List<Album>> pageObject = action.getImageAlbumList(session, request, response);
...
<html lang="<%=locale.toLanguageTag()%>">
  ...
  <body>
    ...
    <div class="container">
      <%=action.getRowHtml(albums, 7, 1, Constant.ALBUM_IMAGE, Constant.ALBUM_IMAGE_DEFAULT_IMAGE, "imageview.jsp") %>

      <%=action.getRowHtml(albums, 7, 2, Constant.ALBUM_IMAGE, Constant.ALBUM_IMAGE_DEFAULT_IMAGE, "imageview.jsp") %>
      </div>
