	...
	$.fn.justifiedGallery = function (arg) {
		...
		function prepareBuildingRow(context, isLastRow) {
			...
			for (i = 0; i < context.buildingRow.entriesBuff.length; i++) {
				...
				if ( newImgW > 300 ) {
					//modify by jack 2015/05/04 , if there is only one image in the gallery ,it should resize it as the height is row height.  
					newImgW = parseInt(context.settings.rowHeight * newImgW / newImgH);
					newImgH = context.settings.rowHeight;
					//console.log(context.settings.rowHeight + " ," + newImgW + " ," + newImgH);
				}
		...
		function analyzeImages(context, isForResize) {
			...
			for (var i = context.lastAnalyzedIndex + 1; i < context.entries.length; i++) {
				...
				//DEBUG// console.log('checking: ' + i + ' (loaded: ' + $image.data('jg.loaded') + ')' +" , src = " + $image.attr("src"));
				
				//log:
				//console.log('checking: ' + i + ' (loaded: ' + $image.data('jg.loaded') + ')' +" , src = " + $image.attr("src"));
		...
		return this.each(function (index, gallery) {
			...
			//log:
			//initial code: context.entries = $gallery.find('> a, > div').toArray();
			context.entries = $gallery.find('> a').toArray();//change by jack
			/*
			for ( var m = 0 ; m < context.entries.length ; m++ ){
				console.log(context.entries[m]);
			}
			*/
