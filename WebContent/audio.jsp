...
IFore action = (IFore)ActionFacotry.getInstance().getAction(SwitchAction.FORE_AUDIO_LIST);
Pages<List<Album>> pageObject = action.getAudioAlbumList(session, request, response);
...
<html lang="<%=locale.toLanguageTag()%>">
  ...
  <body>
    ...
    <div class="container">
      <%=action.getRowHtml(albums, 7, 1, Constant.ALBUM_AUDIO, Constant.ALBUM_AUDIO_DEFAULT_IMAGE, "audioplay.jsp" + pageStr) %>

      <%=action.getRowHtml(albums, 7, 2, Constant.ALBUM_AUDIO, Constant.ALBUM_AUDIO_DEFAULT_IMAGE, "audioplay.jsp" + pageStr) %>
      </div>
